...
|---|---|---|---|---|---|
| any pixel (>0%)   | 915 | 0.882 | 0.033 | 0.063 | 0.033 |
| >= 25% of cell    | 116 | 0.441 | 0.129 | 0.200 | 0.111 |
| >= 50% of cell    |  35 | 0.235 | 0.229 | 0.232 | 0.131 |
| >= 75% of cell    |   9 | 0.147 | 0.556 | 0.233 | 0.132 |

**Interpretation:** high-precision, low-recall coarse detector. 88% of model detections
are confirmed by GFW; for wholesale clearings (>=75% of a cell) recall is 56%. Low recall
...
> Re-scored 2026-07-12 with the corrected `validate_gfw` (inclusive `>= f` reference rule,
> and a read window explicitly clipped to the GFC tile). The `>=25%` and `>=50%` rows are
> unchanged; the `>0%` and `>=75%` rows moved a little because the boundary cells they hinge
> on are exactly the ones the old strict `>` rule dropped (the `>=75%` reference grew from
> 7 cells to 9, so recall reads 0.556 rather than 0.714). The interpretation is the same.
...
1. **Finer spatial sampling** - slide the 64px (640 m) window with a 32px stride
   ...
   |---|---|---|---|
   | >=25% | 0.200 | **0.231** | 0.129 -> 0.156 |
   | >=50% | 0.232 | **0.256** | 0.229 -> 0.270 |
...
AdaBN rescues the Congo Basin site from F1 0.001 to 0.397. Metrics: da_ablation.json.
...
F1 carries a **95% BLOCK-bootstrap CI** (1,000 resamples of 2x2 cell blocks, seed 42);
across-site differences use a **Wilcoxon signed-rank** test (n=8), and spread is the **sample**
sd (n-1).

> **Statistics correction (2026-07-12).** The CIs here were previously computed by resampling
> grid cells i.i.d. With a 32px stride and 64px patches, each ground pixel falls in up to four
> cells, so the cells are not independent and that bootstrap was over-confident: measured at
> Tshopo, the naive interval [0.374, 0.417] widens to [0.345, 0.427] once resampling respects
> the overlap, roughly 1.9x. All intervals below now resample 2x2 blocks of cells. Point
> estimates also shifted a little because the reference threshold is now inclusive (`>= f`,
> matching what we report) rather than strict (`> f`). **The headline conclusion is unchanged:**
> the Tshopo CNN and AdaBN intervals still do not overlap.
...
### Headline table - F1 with 95% block-bootstrap CI (GFW loss-frac >= 25%)

| site | biome / country | CNN F1 | CNN+AdaBN F1 | NDVI F1 |
|---|---|---|---|---|
| Rondonia | Amazon (Brazil) | 0.234 [0.17-0.30] | 0.242 [0.17-0.31] | **0.495 [0.42-0.56]** |
| Sao Felix do Xingu | Amazon (Brazil) | 0.232 [0.19-0.27] | 0.229 [0.19-0.27] | **0.304 [0.26-0.35]** |
| Mato Grosso | Amazon (Brazil) | 0.479 [0.40-0.56] | 0.486 [0.39-0.57] | **0.711 [0.64-0.77]** |
| Riau, Sumatra | Peat / palm oil (Indonesia) | **0.248 [0.21-0.28]** | 0.222 [0.19-0.26] | 0.177 [0.14-0.21] |
| Tshopo | Congo Basin rainforest (DRC) | 0.001 [0.00-0.00] | **0.397 [0.37-0.43]** | 0.220 [0.19-0.25] |
| Mai-Ndombe | Congo Basin rainforest (DRC) | 0.000 [0.00-0.00] | 0.198 [0.17-0.23] | **0.384 [0.32-0.45]** |
| Santa Cruz | Chiquitano dry forest / soy (Bolivia) | **0.156 [0.11-0.20]** | 0.126 [0.08-0.17] | 0.010 [0.00-0.03] |
| Gran Chaco | Dry forest (Paraguay) | 0.000 [0.00-0.00] | 0.100 [0.05-0.15] | **0.392 [0.28-0.49]** |

**Aggregate (mean +/- sample sd across sites):**

| method | F1@25% | P@25% | R@25% | F1@50% |
|---|---|---|---|---|
| CNN | 0.169 +/- 0.168 | 0.273 | 0.132 | 0.148 +/- 0.155 |
| CNN+AdaBN | 0.250 +/- 0.130 | 0.438 | 0.207 | 0.226 +/- 0.163 |
| **NDVI (Otsu)** | **0.337 +/- 0.213** | **0.639** | 0.256 | 0.328 +/- 0.199 |

Per-site wins @25%: NDVI 5, CNN 2, CNN+AdaBN 1. Wilcoxon (n=8): adabn-vs-cnn p=0.31,
adabn-vs-ndvi p=0.20, **cnn-vs-ndvi p=0.055** (W=4) - the plain CNN is **marginally
significantly worse than the NDVI baseline** across sites, and no other pair is significant.
The strongest, unambiguous effects remain at the per-site cell level (non-overlapping
block-bootstrap CIs), below.

### Findings
1. **The plain CNN fails catastrophically out-of-biome - reproducibly - yet excels in-domain.**
   At 3 of 8 sites the unadapted CNN scores F1 <= 0.001: Tshopo (precision 0.022, 46 detections
   of which 1 is correct), Mai-Ndombe (**0 detections**), and Gran Chaco (**0 detections**) - it
   classifies most non-European forest as non-forest, so it finds no Forest->non-Forest
   transitions. Both independent Congo sites fail, so this is a systematic domain-shift failure,
   not a fluke. But at **Mato Grosso** - a mechanized Amazon soy/pasture frontier with large,
   clean clearings, closest to EuroSAT's domain - the plain CNN scores its best (F1 **0.479**,
   precision 0.574, recall 0.412). So the CNN is *bimodal*: strong where the domain is
   near-EuroSAT, near-zero where it is far. This drives its huge spread (+/-0.168).
2. **AdaBN reliably eliminates the catastrophic failures (the key result).** Recomputing
   ...
   positive F1: Tshopo 0.001->**0.397** (precision 0.022->0.702, 672/958 detections correct;
   non-overlapping CIs [0.00-0.00] vs [0.37-0.43]), Mai-Ndombe 0.000->0.198, Gran Chaco
   0.000->0.100. All three AdaBN intervals clear their CNN counterparts. Recovery quality is
   *site-dependent* - full at Tshopo, noisier at Mai-Ndombe (1,626 detections vs 570 reference
   cells: recall 0.381, precision 0.134), modest at Gran Chaco. AdaBN is the **most consistent**
   detector (lowest spread, +/-0.130) and never fails catastrophically; it is
   neutral-to-slightly-positive on the easy sites (Mato Grosso 0.479->0.486) and costs a couple
   of points at Riau and Santa Cruz.
3. **NDVI has the highest mean F1 (0.337) and wins 5/8 sites, but is itself fragile.** It is
   strongest on the Amazon (Mato Grosso 0.711, Rondonia 0.495) and wins **both** Congo sites and
   Gran Chaco - yet **collapses at Santa Cruz** (F1 0.010: 2 detections against 383 reference
   loss cells; both correct, so precision reads 1.000, which means nothing beside a recall of
   0.005). So NDVI's dry-forest behavior is *site-specific, not biome-uniform*: it works at Gran
   Chaco (clearing produces a large NDVI drop the Otsu rule catches) but fails at Santa Cruz (the
   compressed dynamic range defeats the absolute >=0.2-drop rule). Highest spread (+/-0.213).
4. **On dry forest the two methods trade wins** (Santa Cruz: CNN 0.156 > NDVI 0.010; Gran Chaco:
   NDVI 0.392 > CNN 0.000), underscoring that no single detector is safe across sites. Across all
   sites the plain CNN is **marginally significantly worse than NDVI** (Wilcoxon p=0.055).
...
is large" gate. The GFW reference window (2016, 2024] excludes loss Hansen labels 2016, since
composite A is a mid-2016 median; clearing in late 2016 is invisible in A yet also absent from
the reference, so a detector that catches it is charged a false positive. Data:
multi_site_results.json; diagnostics in diagnose_composites.py; change maps in
ml-data/deforestation/multi/.
