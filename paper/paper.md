...
*Working draft. Bracketed `[verify]` notes mark spots to finalize before submission.*

---

## Abstract

Automated deforestation monitoring needs tools that are accurate, reproducible, and cheap to run, but the accuracy a model reports on a benchmark rarely survives contact with new geographies. We study, end to end, how a lightweight land-cover classifier behaves when it is pushed out of its training distribution and across biomes. We fine-tune a ResNet50 (ImageNet-pretrained) on the EuroSAT Sentinel-2 dataset, reaching **98.26%** test accuracy (macro-F1 0.982), and show that a 13-band multispectral variant gives **no measurable gain** (98.22%)—RGB already saturates the benchmark. We then apply the classifier to two-date (2016 vs 2024) Sentinel-2 imagery at **five deforestation frontiers across three biomes and four countries** (Amazon: Rondônia and São Félix do Xingu, Brazil; Indonesian peat/palm oil: Riau, Sumatra; Congo Basin: Tshopo, DRC; dry forest: Santa Cruz, Bolivia), validating detected Forest→non-Forest change against Global Forest Watch (Hansen Global Forest Change) with bootstrap confidence intervals. Two findings drive the paper. First, **benchmark accuracy does not transfer for free**: a naïve application of the EuroSAT model to atmospherically corrected Level-2A imagery misclassifies 72% of rainforest as water until per-channel radiometric *moment matching* is applied, and even after matching the classifier **fails catastrophically in the Congo Basin** (F1 0.001) despite a verifiably clean composite—a genuine domain-shift failure. Second, a simple, **label-free domain adaptation—AdaBN**, recomputing BatchNorm statistics on each target scene—**recovers this failure** (Congo F1 0.001→0.395, precision 0.02→0.70, non-overlapping bootstrap CIs) and yields the most *consistent* detector across biomes (mean F1 0.243±0.087). Against a classical NDVI-difference baseline (with a per-scene Otsu threshold), no method dominates: NDVI is strongest on wet evergreen forest but collapses on dry forest, while the adapted CNN is the most robust. We argue that **cross-biome robustness, not peak in-biome F1, is the metric that matters**, and that a domain-shifted RGB CNN is not a free upgrade over a spectral index without adaptation.
...
Most student and tutorial treatments stop at a single, favorable study area and report agreement with GFW there. We deliberately do the opposite: we deploy the *same* classifier across five frontiers spanning three biomes, look for where it breaks, diagnose why, and test a cheap fix. This turns a "does it work?" demonstration into a small but rigorous **robustness and domain-adaptation study**.
...
3. A **cross-biome evaluation** of the pipeline across five frontiers / three biomes, benchmarked against GFW with bootstrap confidence intervals, exposing a **catastrophic out-of-biome failure** (Congo Basin) that we show—via composite quality diagnostics—is genuine domain shift, not data degradation (§3.4).
...
We selected five documented deforestation frontiers, confirmed on the GFW tree-cover-loss layer, spanning three biomes and four countries:
...
**Composite quality control.** Because a poor result can reflect either the model or the data, we compute per-scene diagnostics (valid-pixel fraction, nodata, saturation, mean NDVI, brightness). All ten composites are clean: 100% valid, ~0% nodata, <2.5% saturated. In particular the Congo scene has healthy mean NDVI 0.66 and 75% high-vegetation cover, so the Congo classifier failure in §3.4 is attributable to domain shift, not clouds or gaps.
...
We evaluate three detectors—CNN, CNN+AdaBN, and NDVI(Otsu)—at all five sites against GFW (Figure 2; Table 1). The results overturn a simple "CNN detects deforestation" narrative.
...
**(a) The CNN fails catastrophically out-of-biome.** In the Congo Basin the plain CNN scores F1 0.001 (precision 0.022): it labels most dense rainforest as non-forest. The composite is clean (§2.3), so this is genuine domain shift—the most important negative result in the paper, and one that single-site studies never surface.

**(b) AdaBN recovers the failure.** Label-free BatchNorm adaptation lifts Congo to F1 **0.395**, precision **0.699**, recall 0.275; the bootstrap CIs do not overlap ([0.00–0.00] vs [0.37–0.42]), so the recovery is unambiguous at the cell level. Congo goes from the worst site to AdaBN's best—reproducing §3.3's controlled result on real imagery.

**(c) AdaBN buys robustness, not a universal win.** It raises cross-biome mean F1 0.174→0.243 and mean precision 0.32→0.49 while *reducing* variance, but is mildly negative on three already-working sites. AdaBN helps most exactly where shift is most severe; a "adapt only if the shift is large" gate is a natural refinement.

**(d) NDVI is the strongest single method on wet forest but the most biome-fragile.** It wins both Amazon sites (0.50, 0.30) yet fails on dry forest (0.010)—*even with the adaptive Otsu threshold*—because NDVI-difference assumes a large absolute NDVI drop on clearing, which fails when deciduous-forest NDVI is already low (scene mean ≈0.42). NDVI has the highest variance (±0.158). The CNN, not tied to a greenness cutoff, degrades more gracefully there.

**Statistical honesty.** With n=5 sites, no pairwise across-site difference is significant (Wilcoxon: AdaBN-vs-CNN p=0.81, CNN-vs-NDVI p=0.44; per-site wins CNN 2 / AdaBN 1 / NDVI 2). The robust, significant effects are per-site (non-overlapping cell-bootstrap CIs), above. We therefore claim *robustness and biome-dependence*, not a single winning method.

## 4. Discussion

The headline lesson is that **in-benchmark accuracy is a poor predictor of cross-biome behavior**. A 98%-accurate classifier fails two ways in the wild—silently (radiometric shift → forest as water) and catastrophically out-of-biome (Congo)—and single-site evaluation hides both. Deploying the same model across biomes and *looking for where it breaks* is what makes the failures visible and the fix testable.

The fix is cheap. AdaBN needs no labels, no retraining, and one forward pass, yet it converts the CNN's worst site into a solid one and makes the detector the most consistent across biomes. This is a favorable trade for an operational screening tool, where a catastrophic blind spot in one biome is far worse than a few points of F1 elsewhere.

The NDVI comparison keeps the CNN honest: a classical index beats it on wet evergreen forest and should not be dismissed. The useful synthesis is that the two are complementary—NDVI excels where the spectral signal of clearing is strong; the adapted CNN is more robust where it is not (plantation regrowth, dry forest, unseen radiometry).

**Limitations.** (i) n=5 sites limits across-site statistical power; more frontiers per biome would enable proper significance testing. (ii) 640 m patches miss small/linear clearings, capping recall against all GFW loss (recall is highest for wholesale clearings). (iii) The NDVI baseline uses an absolute-drop rule; a relative-drop variant might be fairer in dry forest, though absolute NDVI-difference is the textbook method. (iv) Moment matching approximates, and AdaBN adapts only second-order (BN) statistics—deeper shift (label/prior shift, unseen classes) is unaddressed. (v) The pipeline classifies Sentinel-2 with a model trained on European scenes; region-matched labels would reduce the underlying shift.

## 5. Conclusion and Future Work

A lightweight, fully reproducible ResNet50/EuroSAT pipeline can recover a deforestation signal that agrees with Global Forest Watch—but only within its comfort zone. Pushed across biomes it fails outright in the Congo Basin, a failure we attribute to genuine domain shift and repair with a label-free AdaBN pass, yielding the most consistent detector across three biomes while a classical NDVI baseline remains the strongest on wet evergreen forest. **Robustness across biomes, not peak in-biome F1, is the metric that matters.** Future work: (1) more sites per biome for statistical power; (2) a shift-magnitude gate for selective adaptation; (3) region-matched fine-tuning and deeper adaptation (e.g., feature alignment) beyond BatchNorm; (4) finer patches or segmentation for sub-patch recall; (5) multi-temporal sequences beyond two snapshots.
